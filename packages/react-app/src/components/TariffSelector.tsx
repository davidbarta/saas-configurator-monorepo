...
import { TranslationKey } from '@saas/locales';
...
export default function TarrifSelector() {
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-lg font-semibold text-slate-800">{t('selector.title')}</h2>
          ...
          >
            <div>
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-lg text-slate-900">
                  {t(tariff.nameKey as TranslationKey)}
                </h3>
                {isSelected(tariff) && (
              ...
              <p className="text-sm text-slate-500 min-h-10">
                {t(tariff.descriptionKey as TranslationKey)}
              </p>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex items-baseline justify-between">
              ...
              <button
                ...
                aria-label={
                  isSelected(tariff)
                    ? t('selector.selectedTariff', { tariff: tariff.nameKey })
                    : t('selector.chooseTariff', { tariff: tariff.nameKey })
                }
